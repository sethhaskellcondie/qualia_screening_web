 ...
 * NOTE TO CANDIDATE: 
 * 
 * The specific implementation details of this file are not very important. 
 * What's important is the public contract, which is well documented in the 
 * in the README. Scanning this file will not be a good use of your limited time.
 */

/**
 * Bounces the price around by a reasonable amount
 */
function randomlyMovePrice(price: number): number {
  const change = Math.random() * 0.1 - 0.05;
  return Math.floor(price * (1 + change));
}
...
/**
 * Returns a random price in cents, between 1000 and 30000
 */
function getRandomPrice(): number {
  const CENTS_IN_DOLLAR = 100;
  return randomNumberBetween(10 * CENTS_IN_DOLLAR, 300 * CENTS_IN_DOLLAR);
}

/**
 * Returns a random price-to-earnings ratio between 10 and 100
 */
function getRandomPriceToEarnings(): number {
  return randomNumberBetween(10, 100);
}
...
const stocks: Array<{ symbol: string; name: string }> = [
  ...
  { symbol: "DLPH", name: "Delphi Automotive" },
  ...
  { symbol: "RE", name: "Everest Re Group" },
  ...
  { symbol: "Q", name: "Quintiles IMS Holdings, Inc" },
...
export interface Company {
  ...
  name: string;
  /** The last trading price of the stock, represented in cents */
  lastPrice: number;
  /** The company's price-to-earnings ratio */
  priceToEarnings: number;
  /** The company's market capitalization, represented in cents */
  marketCap: number;
...
/**
 * A callback that is called when the stock prices change for watched companies.
 */
export type OnDataChangedCallback = (companies: Company[]) => void;

export class CompanyNotFoundError extends Error {
  companySymbol: CompanySymbol;
  
  constructor(message: string, companySymbol: CompanySymbol) {
    super(message);
...
/**
 * A service that provides stock prices and other information.
 */
export class TickerService {
  /**
   * An inventory of active "subscriptions"
   */
  private subscriptions = new Set<CompanySymbol>();

  /**
   * All of the company data, with mock pricing.
   */
   ...
   * A callback that is called when the stock prices change for watched companies.
   */
  private onDataChangedFns: OnDataChangedCallback[] = [];

  constructor() {
    this.data = stocks.map((company) => {
      const lastPrice = getRandomPrice();
      const sharesOutstanding = randomNumberBetween(1_000_000, 1_000_000_000);
      const marketCap = lastPrice * sharesOutstanding;
      return {
        ...company,
        priceToEarnings: getRandomPriceToEarnings(),
        lastPrice,
        marketCap,
        sharesOutstanding,
      };
    ...
    // Every second, update the price and market cap of all companies
    setInterval(() => {
      this.data.forEach((company) => {
        company.lastPrice = randomlyMovePrice(company.lastPrice);
        company.marketCap = company.lastPrice * company.sharesOutstanding;
      });

      // Only call the callback if there are any subscriptions
      if (this.onDataChangedFns.length && this.subscriptions.size > 0) {
        const companies = this.data.filter((company) =>
          this.subscriptions.has(company.symbol.toLowerCase())
        );

        this.onDataChangedFns.forEach((fn) => fn(companies));
      }
    }, 300);
  }

  /**
   * Given a search term, searches the company symbol and name for matches.
   *
   * @param searchStr A case-insensitive string to search for in the company symbol or name.
   * @returns A promise which resolves to the search term and an array of matching companies.
   */
  search(searchStr: string): Promise<{ search: string; results: Company[] }> {
    const normalizedSearchStr = searchStr.toLowerCase().trim();

    const predicate = (company: Company): boolean => {
      const symbolMatch = company.symbol
        .toLowerCase()
        .includes(normalizedSearchStr);
    ...
    // Add a random delay to simulate network chop.
    // This is to help the interviewee understand that
    // they should not assume the data is immediately available,
    // an may not come back in the same order as requested.
    return new Promise((resolve) => {
      setTimeout(() => {
        resolve({
          search: searchStr,
          results: this.data.filter(predicate),
        });
      }, randomNumberBetween(50, 1000));
    });
  }

  /**
   * Instructs the service to begin watching a given symbol.
   *
   * When a symbol is watched, the service will periodically update
   * the price and market cap via the `onDataChanged` callback.
   *
   * @param symbol The ticker symbol to watch
   * @throws {CompanyNotFoundError} When the company is not found
   * @throws {AlreadySubscribedError} When the symbol is already subscribed to
   */
  watch(symbol: CompanySymbol): void {
    const company = this.data.find(
      (company) => company.symbol.toLowerCase() === symbol.toLowerCase()
    );

    if (!company) {
      throw new CompanyNotFoundError(`Company with symbol not found`, symbol);
    }

    if (this.subscriptions.has(symbol.toLowerCase())) {
      throw new AlreadySubscribedError(`Already subscribed`, symbol);
    }

    this.subscriptions.add(symbol.toLowerCase());
  }

  /**
   * The opposite of `watch`. Stops watching a symbol.
   *
   * When called, the service will no longer update pricing data via the `onDataChanged` callback.
   *
   * When called without a symbol, all subscriptions will be removed.
   *
   * @param symbol The symbol to stop watching, or undefined to remove all subscriptions
   */
  unwatch(symbol?: CompanySymbol): void {
    if (!symbol) {
      this.subscriptions.clear();
      return;
    }

    this.subscriptions.delete(symbol.toLowerCase());
  }

  /**
   * Registers a callback that will be called when the data changes for any watched symbol.
   *
   * All watched symbols will be passed to the callback.
   *
   * @param callback The callback function to register
   */
  onDataChanged(callback: OnDataChangedCallback): void {
    // Just a little help for the interviewee, in case they pass the wrong type
    if (typeof callback !== "function") {
      throw new Error("Callback must be a function");
    }

    if (this.onDataChangedFns.includes(callback)) {
      throw new Error("Callback already registered");
    }

    if (this.onDataChangedFns.length > 0) {
      console.warn(
        'More than one callback registered for "onDataChanged". ' +
          "This likely indicates a memory leak. Developer hint: " +
          "If you are using React please disable Strict Mode in your main/index file "
      );
    }

    this.onDataChangedFns.push(callback);
  }

  /**
   * Checks if the service is subscribed to a symbol.
   *
   * @param symbol The symbol to check
   * @returns True if the service is subscribed to the symbol, false otherwise
   */
  isWatched(symbol: CompanySymbol): boolean {
    return this.subscriptions.has(symbol.toLowerCase());
  }
}
